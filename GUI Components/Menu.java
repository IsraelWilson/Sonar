...
{
    private static Font font = new Font("Times-Roman", Font.PLAIN, 12);
    ...
    public void act() 
    {
        ...
        else
        {
            for (int i = 0; i < items.size(); i++)
                if (active.startsWith(items.get(i).getName() + "/"))
                {
                    ...
                    height += 12 + items.get(i).getRunningHeight(active.substring(active.indexOf("/") + 1));
                    break;
                }
                else
        ...
        if (origin != null)
            home = new Point((int)origin.getX() + barWidth / 2, (int)origin.getY() + 12 / 2);
        
        setLocation((int)home.getX() + (width - barWidth) / 2, (int)home.getY() + (height - 12) / 2);
        ...
        if (clicked && offsetMouse.getY() < 12)//offsetMouse.getX() > 0 && offsetMouse.getX() < width && offsetMouse.getY() > 0 && offsetMouse.getY() < 12)
        {
            int runX = 0;
            for (MenuItem mi : items)
        ...
        for (MenuItem barItem : items)
        {
            ...
            if (!active.isEmpty() && offsetMouse.getX() > runWidth && offsetMouse.getX() <= runWidth + barItem.getWidth() && offsetMouse.getY() > 0 && offsetMouse.getY() <= 12)
            {
            ...
            if (active.startsWith(barItem.getName() + "/"))
            {
                ...
                while (!s.isEmpty() || sneakpeek)
                {
                    ...
                    for (int i = 0; i < item.getItems().size(); i++)
                    {
                        ...
                        if (offsetMouse.getX() > runningX && offsetMouse.getX() <= runningX + w && offsetMouse.getY() > runningY + i * 12 && offsetMouse.getY() <= runningY + i * 12 + 12)
                        {
                        ...
                        pic.fillRect(runningX, runningY + i * 12, w, 12);
                        ...
                        if (!item.getItems().get(i).getItems().isEmpty())
                        {
                            if (item.getItems().get(i).isEnabled())
                                ...
                                pic.setColor(new Color(220, 220, 220));
                            int[] xs = new int[3];
                            xs[0] = runningX + w - 8;
                            xs[1] = runningX + w - 1;
                            xs[2] = runningX + w - 8;
                            int[] ys = new int[3];
                            ys[0] = runningY + i * 12 + 2;
                            ys[1] = runningY + i * 12 + 6;
                            ys[2] = runningY + i * 12 + 10;
                            pic.fillPolygon(xs, ys, 3);
                        }
                        
                        // If checked, draw checkmark.
                        if (item.getItems().get(i).isChecked() && runningY + i * 12 + 2 + 8 < pic.getHeight())
                        {
                            pic.setColorAt(runningX + 2 + 6, runningY + i * 12 + 2 + 0, new Color(121, 121, 121));
                            pic.setColorAt(runningX + 2 + 7, runningY + i * 12 + 2 + 0, new Color(0, 0, 0));
                            pic.setColorAt(runningX + 2 + 8, runningY + i * 12 + 2 + 0, new Color(194, 194, 194));
                            pic.setColorAt(runningX + 2 + 5, runningY + i * 12 + 2 + 1, new Color(138, 138, 138));
                            pic.setColorAt(runningX + 2 + 6, runningY + i * 12 + 2 + 1, new Color(0, 0, 0));
                            pic.setColorAt(runningX + 2 + 7, runningY + i * 12 + 2 + 1, new Color(189, 189, 189));
                            pic.setColorAt(runningX + 2 + 5, runningY + i * 12 + 2 + 2, new Color(0, 0, 0));
                            pic.setColorAt(runningX + 2 + 6, runningY + i * 12 + 2 + 2, new Color(137, 137, 137));
                            pic.setColorAt(runningX + 2 + 4, runningY + i * 12 + 2 + 3, new Color(138, 138, 138));
                            pic.setColorAt(runningX + 2 + 5, runningY + i * 12 + 2 + 3, new Color(27, 27, 27));
                            pic.setColorAt(runningX + 2 + 1, runningY + i * 12 + 2 + 4, new Color(43, 43, 43));
                            pic.setColorAt(runningX + 2 + 2, runningY + i * 12 + 2 + 4, new Color(168, 168, 168));
                            pic.setColorAt(runningX + 2 + 4, runningY + i * 12 + 2 + 4, new Color(28, 28, 28));
                            pic.setColorAt(runningX + 2 + 5, runningY + i * 12 + 2 + 4, new Color(151, 151, 151));
                            pic.setColorAt(runningX + 2 + 1, runningY + i * 12 + 2 + 5, new Color(122, 122, 122));
                            pic.setColorAt(runningX + 2 + 2, runningY + i * 12 + 2 + 5, new Color(0, 0, 0));
                            pic.setColorAt(runningX + 2 + 3, runningY + i * 12 + 2 + 5, new Color(93, 93, 93));
                            pic.setColorAt(runningX + 2 + 4, runningY + i * 12 + 2 + 5, new Color(16, 16, 16));
                            pic.setColorAt(runningX + 2 + 2, runningY + i * 12 + 2 + 6, new Color(42, 42, 42));
                            pic.setColorAt(runningX + 2 + 3, runningY + i * 12 + 2 + 6, new Color(0, 0, 0));
                            pic.setColorAt(runningX + 2 + 4, runningY + i * 12 + 2 + 6, new Color(93, 93, 93));
                            pic.setColorAt(runningX + 2 + 2, runningY + i * 12 + 2 + 7, new Color(190, 190, 190));
                            pic.setColorAt(runningX + 2 + 3, runningY + i * 12 + 2 + 7, new Color(0, 0, 0));
                            pic.setColorAt(runningX + 2 + 4, runningY + i * 12 + 2 + 7, new Color(190, 190, 190));
                            pic.setColorAt(runningX + 2 + 3, runningY + i * 12 + 2 + 8, new Color(131, 131, 131));
                        }
                        
                        pic.setColor(Color.BLACK);
                        
                        // Draw frame of MenuItem.
                        pic.drawRect(runningX, runningY + i * 12, w, 12);
                        
                        // Draw MenuItem text.
                        if (!item.getItems().get(i).isEnabled())
                            pic.setColor(Color.GRAY);
                        pic.drawString(item.getItems().get(i).getName(), runningX + 2 + (item.getItems().get(i).isMemberOfSet() ? checkMarkWidth : 0), runningY + i * 12 + 11);
                        if (item.getItems().get(i).getShortCut() != null)
                            pic.drawString(item.getItems().get(i).getShortCut(), runningX + w - item.getItems().get(i).getWidthOfShortCut() - 2, runningY + i * 12 + 11);
                        //pic.drawString((item.getItems().get(i).isChecked() ? "? " : "") + item.getItems().get(i).getName(), runningX + 2 + (item.getItems().get(i).isMemberOfSet() && !item.getItems().get(i).isChecked() ? checkMarkWidth : 0), runningY + i * 12 + 11);
                    }
            ...
            pic.fillRect(runWidth, 0, barItem.getWidth(), 12);
            pic.setColor(Color.BLACK);
            // Frame.
            pic.drawRect(runWidth, 0, barItem.getWidth(), 12);
            // Text.
            pic.drawString(barItem.getName(), runWidth + 2, 11);
    ...
    public void addedToWorld(World world)
    {
        if (origin != null)
        {
            ...
            for (MenuItem mi : items)
                barWidth += mi.getWidth();
            home = new Point((int)origin.getX() + barWidth / 2, (int)origin.getY() + 12 / 2);
            setLocation((int)home.getX() + (getImage().getWidth() - barWidth) / 2, (int)home.getY() + (getImage().getHeight() - 12) / 2);
        }
        else
            home = new Point(getX(), getY());
    }
