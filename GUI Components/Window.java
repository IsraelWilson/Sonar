...
public abstract class Window extends GUI_Component
{
    ...
    public static void setDefaultColorScheme(Color back, Color text)
    {
    ...
    {
        for (Object o : WorldHandler.getInstance().getWorld().getObjects(Window.class))
            ((Window)o).bringToFront();
    }
    ...
    public Window(String title, int w, int h, boolean alwaysOpenToDefault, boolean closeWhenLoseFocus)
    {
        ...
        btnClose.setColorScheme(new Color(0, 0, 0, 0), textColor, new Color(125, 125, 125, 80), Color.GRAY, Color.GRAY);
    }
    ...
    protected void resizeWidthForTitle(String t, int minWidth)
    {
        title = t;
        Graphics2D g = (Graphics2D)(new GreenfootImage(1, 1)).getAwtImage().getGraphics();
        FontMetrics fm = g.getFontMetrics();
        int tw = fm.charsWidth(t.toCharArray(), 0, t.length());
        g.dispose();
        int w = minWidth;
        if (w < tw + 20)
            w = tw + 20;
        if (w > WorldHandler.getInstance().getWorld().getWidth())
            w = WorldHandler.getInstance().getWorld().getWidth();
        size = new Point(w, (int)size.getY());
        setImage(draw());
