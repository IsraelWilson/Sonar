 ...
 * An "Enter" and "Cancel" Button that both remove the Palette from the World. The "Enter" Button provokes the action listener method getResult() to return the last selected Color, while the "Cancel" Button does not.<p>
 ...
 * 
 * @author Taylor Born
 * @version November 2011 - March 2013
 */
public class Palette extends Window
{
    ...
    private Button btnEnter = new Button("Enter", new Point(44, 22));
    ...
    public void act()
    {
        ...
        {
            // Set result.
            result = colors[selected].getColor();
            toggleShow();
        }
        if (btnCancel.wasClicked())
            toggleShow();
    }
    ...
    /**
     * Action listener for the Palette. When "Enter" Button is clicked, will return Color last selected.<p>
     * Resets once called. (Get one result with one use of "Enter" Button).
     * @return Color last selected. Null if "Enter" Button was not clicked.
     */
    ...
    private class ColorMemoryCell extends WindowComponent
    {
        ...
        {
            this.c = c;
            GreenfootImage pic = new GreenfootImage(13, 13);
            pic.setColor(c);
