...
public class TextBox extends Window
{
    ...
    public TextBox(Point size, String text, Font font)
    {
        ...
        if (size.getY() < font.getSize() + 6)
            size = new Point((int)size.getX(), font.getSize() + 6);
        this.size = size;
        this.text = text;
        this.font = font;
        caret = text.length();
    ...
    {
        this(size, text, new Font("Times-Roman", Font.PLAIN, 12));
    }
    ...
    private void handleInput()
    {
        ...
        if (s != null && !readOnly && s.charAt(0) >= 32 && s.charAt(0) <= 127)
        {
            ...
            else
            {
                ...
                if (s.length() == 1 && (maxLength == -1 || text.length() < maxLength) && (accept.equals("") || accept.contains(s)) && !dontAccept.contains(s))
                {
                    if (selectingPivot != -1)
                    {
                        text = text.substring(0, Math.min(caret, selectingPivot)) + text.substring(Math.max(caret, selectingPivot));
                ...
                if (caret >= strs.get(0).length())
                {
                    ...
                    for (int i = 0; i < strs.size(); i++)
                    {
                        if (l + strs.get(i).length() > caret)
                        {
                            ...
                            if (lastCursorX == null)
                            {
                                cur = 4 + fm.charsWidth(strs.get(i).substring(0, caret - l).toCharArray(), 0, caret - l);
                                lastCursorX = cur;
                            }
                            else
                                cur = lastCursorX;
    ...
    /**
     * Set String of characters that will be allowed as input to be only upper/lower case letters and digits 0-9.
     */
    public void acceptLettersAndNumbersOnly()
    {
        accept = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
    }
    ...
    private class DrawFunction
    {
        ...
        public void paint()
        {
            ...
            while (!remainingBeforeN.isEmpty() || !remainingAfterN.isEmpty())
            {
                ...
                while (!remainingBeforeN.isEmpty())
                {
                    ...
                    int currentWidth = fm.charsWidth(current.toCharArray(), 0, current.length());
                    ...
                    int nextWidth = fm.charsWidth(next.toCharArray(), 0, next.length());
                    ...
                    if (currentWidth + nextWidth > size.getX() - SCROLL_WIDTH - 3)
                    {
                        if (current.isEmpty())
                            ...
                            {
                                if (fm.charsWidth(remainingBeforeN.substring(0, i).toCharArray(), 0, i) > size.getX() - SCROLL_WIDTH - 3)
                                {
                    ...
                    if (needToUpdateRemaining)
                        ...
                        if (current.isEmpty() && nextWidth > size.getX() - SCROLL_WIDTH - 3)
                        {
                            for (int i = 1; i <= next.length(); i++)
                            {
                                if (fm.charsWidth(next.substring(0, i).toCharArray(), 0, i) > size.getX() - SCROLL_WIDTH - 3)
                                {
            ...
            // Draw lines of text.
            pic.setColor(showingMessage ? messageColor : textColor);
            for (int i = 0; i < strs.size(); i++)
                pic.drawString(strs.get(i), 4, font.getSize() * (i + 1) - scrollValue);
        ...
        private void addCurrent_drawHighlight_rememberCaret()
        {
            ...
            if (selectingPivot != -1)
            {
                ...
                if (maxC > count && minC < count + current.length())
                {
                    ...
                    if (maxC > current.length())
                        maxC = current.length();
                    int hBegin = fm.charsWidth(current.substring(0, minC).toCharArray(), 0, minC) + 4;
                    int hLength = fm.charsWidth(current.substring(minC, maxC).toCharArray(), 0, maxC- minC);
            ...
            {
                caretX = fm.charsWidth(current.substring(0, caret - count).toCharArray(), 0, caret - count) + 4;
                caretLine = strs.size();
            }
        }
        
        private void handleCaret_finishCurrent()
        {
            // If looking to place caret within text, and is within current.
            if (clickedAt != null && clickedAt.getY() > font.getSize() * (strs.size() - 1) - scrollValue && clickedAt.getY() <= font.getSize() * strs.size() - scrollValue)
            {
                // If to the left of entire line of text.
                if (clickedAt.getX() < 4 + fm.charsWidth(current.substring(0, 1).toCharArray(), 0, 1) / 2)
                {
                    caret = count;
                    clickedAt = null;
                }
                else
                {
                    ...
                    for (int i = 1; i < current.length() - 1; i++)
                        ...
                        if (clickedAt.getX() >= 4 + fm.charsWidth(current.substring(0, i - 1).toCharArray(), 0, i - 1) + fm.charsWidth(current.substring(i - 1, i).toCharArray(), 0, 1) / 2 && clickedAt.getX() < 4 + fm.charsWidth(current.substring(0, i).toCharArray(), 0, i) + fm.charsWidth(current.substring(i, i + 1).toCharArray(), 0, 1) / 2)
                        {
                            caret = count + i;
                            clickedAt = null;
                            break;
